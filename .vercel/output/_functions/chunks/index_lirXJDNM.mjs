import { aH as ProviderError, aI as IniSectionType, aJ as CONFIG_PREFIX_SEPARATOR, Q as readFile, aK as getConfigFilepath, aL as parseIni, o as getProfileName, C as CredentialsProviderError, s as setCredentialFeature } from "./S3Client_BSd7K461.mjs";
import { promises } from "node:fs";
import { g as getSSOTokenFilepath, a as getSSOTokenFromFile } from "./getSSOTokenFromFile_D0WoEGs3.mjs";
...
const getSsoOidcClient = async (ssoRegion, init = {}, callerClientConfig) => {
  const { SSOOIDCClient } = await import("./index_CJOVicLy.mjs");
...
const getNewSsoOidcToken = async (ssoToken, ssoRegion, init = {}, callerClientConfig) => {
  const { CreateTokenCommand } = await import("./index_CJOVicLy.mjs");
...
const resolveSSOCredentials = async ({ ssoStartUrl, ssoSession, ssoAccountId, ssoRegion, ssoRoleName, ssoClient, clientConfig, parentClientConfig, callerClientConfig, profile, filepath, configFilepath, ignoreCache, logger }) => {
  ...
  const { SSOClient, GetRoleCredentialsCommand } = await import("./loadSso_Cp3Kr8GL.mjs");
