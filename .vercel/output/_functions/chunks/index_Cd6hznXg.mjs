import { C as CredentialsProviderError, O as chain, s as setCredentialFeature, o as getProfileName, Q as readFile, H as HttpRequest } from "./S3Client_BSd7K461.mjs";
import { createHash, createPrivateKey, createPublicKey, sign } from "node:crypto";
import { promises } from "node:fs";
import { homedir } from "node:os";
import { dirname, join } from "node:path";
import { p as parseKnownFiles } from "./parseKnownFiles_DIidHG4o.mjs";
const resolveCredentialSource = (credentialSource, profileName, logger) => {
  const sourceProvidersMap = {
    EcsContainer: async (options) => {
      const { fromHttp } = await import("./index__Y9HRHYj.mjs");
      const { fromContainerMetadata } = await import("./index_Yofz20E_.mjs");
      logger?.debug("@aws-sdk/credential-provider-ini - credential_source is EcsContainer");
      return async () => chain(fromHttp(options ?? {}), fromContainerMetadata(options))().then(setNamedProvider);
    },
    Ec2InstanceMetadata: async (options) => {
      logger?.debug("@aws-sdk/credential-provider-ini - credential_source is Ec2InstanceMetadata");
      const { fromInstanceMetadata } = await import("./index_Yofz20E_.mjs");
      return async () => fromInstanceMetadata(options)().then(setNamedProvider);
    },
    Environment: async (options) => {
      logger?.debug("@aws-sdk/credential-provider-ini - credential_source is Environment");
      const { fromEnv } = await import("./index_B1Er76gH.mjs");
...
const resolveAssumeRoleCredentials = async (profileName, profiles, options, callerClientConfig, visitedProfiles = {}, resolveProfileData2) => {
  ...
  if (!options.roleAssumer) {
    const { getDefaultRoleAssumer } = await import("./index_DG1qUtsP.mjs");
...
class LoginCredentialsFetcher {
  ...
  async refresh(token) {
    const { SigninClient, CreateOAuth2TokenCommand } = await import("./index_DBIiTC2z.mjs");
...
const resolveProcessCredentials = async (options, profile) => import("./index_B6cuPliK.mjs").then(({ fromProcess }) => fromProcess({
  ...options,
  profile
...
const resolveWebIdentityCredentials = async (profile, options, callerClientConfig) => import("./index_b7YA2NIy.mjs").then(({ fromTokenFile }) => fromTokenFile({
