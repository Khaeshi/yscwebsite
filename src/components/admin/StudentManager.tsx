...
export default function StudentsManager({ initialStudents = [] }: Props) {
  ...
  useEffect(() => { if (initialStudents.length === 0) fetchStudents(); }, []);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (editingId) { setStudents(p => p.map(s => s._id === editingId ? data.student : s)); addToast('Student updated'); }
      else { setStudents(p => [data.student, ...p]); addToast('Student added'); }
      closeModal();
  ...
  async function handleDelete(student: Student) {
    setDeleteTarget(null); setDeletingId(student._id);
    try {
    ...
    <div className="text-gray-800 min-h-screen p-3 sm:p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-gray-500 text-xs sm:text-sm mt-0.5">Manage students and their Telegram connections</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-4 mb-5">
        {([
          ...
          <div key={s.label} className="bg-white border border-gray-200 rounded-xl p-3 sm:p-5 shadow-sm">
            <p className="text-gray-500 text-xs font-semibold uppercase tracking-wider mb-1 sm:mb-2 truncate">{s.label}</p>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 sm:py-24 text-center px-4">
            ...
            <p className="text-base font-semibold text-gray-700">{search ? 'No students match your search' : 'No students yet'}</p>
            {!search && (
          ...
          <>
                  ...
                  {filtered.map(s => (
                    <tr key={s._id} className="hover:bg-gray-50/60 transition-colors fade-up">
                      ...
                      <td className="px-5 py-4">
                        {s.telegramChatId ? (
                          <div>
                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-50 text-emerald-600 border border-emerald-100">✓ Connected</span>
                            <p className="text-xs text-gray-400 mt-1 font-mono">{s.telegramChatId}</p>
                          </div>
                        ) : (
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-50 text-amber-600 border border-amber-100">⚠ Not set</span>
                        )}
                      </td>
                      <td className="px-5 py-4">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${s.active ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-gray-100 text-gray-500 border-gray-200'}`}>
                          {s.active ? 'Active' : 'Inactive'}
                        </span>
                      </td>
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-1">
                          <button onClick={() => openEdit(s)} className="p-1.5 rounded-lg hover:bg-purple-50 text-purple-500 transition-colors">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                            </svg>
                          </button>
                          <button onClick={() => setDeleteTarget(s)} disabled={deletingId === s._id} className="p-1.5 rounded-lg hover:bg-red-50 text-red-400 disabled:opacity-30 transition-colors">
                            {deletingId === s._id
            ...
            <div className="sm:hidden divide-y divide-gray-100">
              {filtered.map(s => (
                <div key={s._id} className="p-4 fade-up">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0">
                      <Avatar name={s.name} />
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-gray-900 truncate">{s.name}</p>
                        <p className="text-xs text-gray-500 truncate">{s.phone || s.email || '—'}</p>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-2 mt-3 flex-wrap">
                    {s.telegramChatId
                      ? <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-emerald-50 text-emerald-600 border border-emerald-100">✓ Telegram</span>
                      : <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-amber-50 text-amber-600 border border-amber-100">⚠ No Telegram</span>
                    }
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${s.active ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-gray-100 text-gray-500 border-gray-200'}`}>
                      {s.active ? 'Active' : 'Inactive'}
                    </span>
                  </div>
                </div>
      ...
      <div className="mt-4 bg-blue-50 border border-blue-100 rounded-xl p-4 flex gap-3">
        ...
        <div>
          ...
          <p className="text-blue-600/80 text-xs leading-relaxed">
            Student opens Telegram → finds your bot → sends <code className="bg-blue-100 px-1.5 py-0.5 rounded font-mono">/start</code> → bot replies with their Chat ID → student shares it with you → add it here.
          </p>
        </div>
      </div>
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-white border border-gray-200 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md shadow-xl fade-up max-h-[92vh] overflow-y-auto">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 sticky top-0 bg-white z-10">
              <div>
                <h2 className="text-base font-bold text-gray-900">{editingId ? 'Edit Student' : 'Add Student'}</h2>
            ...
            <form onSubmit={handleSubmit} className="px-5 py-5 space-y-4">
              <div>
                ...
                <input required type="text" placeholder="e.g. Maria Santos" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className={inputCls} />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Phone</label>
                  <input type="tel" placeholder="0917-123-4567" value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} className={inputCls} />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Email</label>
                  <input type="email" placeholder="maria@example.com" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} className={inputCls} />
                </div>
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Telegram Chat ID</label>
                <input type="text" placeholder="123456789" value={form.telegramChatId} onChange={e => setForm({ ...form, telegramChatId: e.target.value })} className={inputCls} />
              ...
              <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
                ...
                <span className="text-sm text-gray-600">{form.active ? 'Active student' : 'Inactive'}</span>
              </div>
              <div className="flex gap-3 pt-1">
                ...
                <button type="submit" disabled={submitting} className="flex-1 px-4 py-2.5 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white rounded-xl text-sm font-semibold transition-colors flex items-center justify-center gap-2">
                  {submitting ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Saving...</> : editingId ? 'Save Changes' : 'Add Student'}
                </button>
              </div>
            </form>
          </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-white border border-gray-200 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-sm shadow-xl fade-up p-6">
            ...
            <p className="text-gray-500 text-sm mb-6">Remove <strong className="text-gray-900">{deleteTarget.name}</strong>? This will also affect their schedules.</p>
            <div className="flex gap-3">
              ...
              <button onClick={() => handleDelete(deleteTarget)} className="flex-1 px-4 py-2.5 bg-red-500 hover:bg-red-600 text-white rounded-xl text-sm font-semibold transition-colors">Remove</button>
            </div>
          </div>
        </div>
      )}
